create table public.password_recovery_rate_limits (
  scope_kind text not null check (scope_kind in ('client', 'recipient')),
  ...
  primary key (scope_kind, scope_sha256)
);

alter table public.password_recovery_rate_limits enable row level security;

revoke all on table public.password_recovery_rate_limits
...
begin
  ...
  if client_limit.scope_sha256 is not null
    and client_limit.window_started_at > current_time - interval '15 minutes'
    and client_limit.attempts >= 8 then
    return false;
  end if;

  if recipient_limit.scope_sha256 is not null
    and recipient_limit.last_attempt_at > current_time - interval '60 seconds' then
    return false;
  end if;
  ...
  values ('client', p_client_sha256, current_time, 1, current_time)
  ...
  values ('recipient', recipient_hash, current_time, 1, current_time)
  on conflict (scope_kind, scope_sha256) do update
  set
  ...
  where last_attempt_at < current_time - interval '7 days';
