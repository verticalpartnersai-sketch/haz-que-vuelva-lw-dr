...
begin
  ...
  select coalesce(max(cf.version), 0) + 1
  into next_version
  from public.content_files as cf
  where cf.content_item_id = item_id;
...
comment on function public.publish_content_pdf(
  text, text, text, text, text, bigint, text
) is
  'Publishes a versioned private product PDF after the reauthentication wrapper validates the owner admin.';
